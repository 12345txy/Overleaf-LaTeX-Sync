...
def login():
    ...
    # Qt WebEngine may not expose GCLB; fetch via HTTP so Socket.IO sees the same cookies as a browser.
    if "GCLB" not in dat["cookie"]:
        try:
            r = reqs.get(SOCKET_URL, cookies=dat["cookie"])
            if "GCLB" in r.cookies:
                dat["cookie"]["GCLB"] = r.cookies["GCLB"]
        except Exception:
            pass
        if "GCLB" not in dat["cookie"]:
            try:
                r = reqs.get(PROJECT_URL, cookies=dat["cookie"])
                if "GCLB" in r.cookies:
                    dat["cookie"]["GCLB"] = r.cookies["GCLB"]
            except Exception:
                pass
